...
export class FilesService {
  ...
  getFiles(): void {
    this.http.get<any[]>(`${this.apiUrl}/files`, { withCredentials: true }).subscribe({
      next: (files) => {
  ...
  createFile(titre: string, file: File): Observable<any> {
    ...
    return new Observable((observer) => {
      this.http.post(`${this.apiUrl}/files`, formData, { withCredentials: true }).subscribe({
        next: (response) => {
  ...
  deleteFile(id: string): Observable<any> {
    return new Observable((observer) => {
      this.http.delete(`${this.apiUrl}/files/${id}`, { withCredentials: true }).subscribe({
        next: (response) => {
